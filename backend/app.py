...
from models import LoginIn, TokenOut, ChangePasswordIn, AlunoIn, AlunoOut, DisciplinaIn, DisciplinaOut, NotaIn
import storage as db
from util import nonempty

app = FastAPI(title="Controle Acadêmico API", version="1.0.0")
ensure_admin()

# ✅ Habilita CORS para o frontend em http://localhost:5173 e http://127.0.0.1:5173
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        "http://localhost:5173",
...
def list_students(
    ...
    return [
        AlunoOut(
            id=a.id, nome=a.nome, tipo_id=a.tipo_id, identificador=a.identificador, data_cadastro=a.data_cadastro,
            disciplinas=[DisciplinaOut(id=d.id, nome=d.nome, data_cadastro=d.data_cadastro, notas=d.notas, media=d.media(), status=d.status()) for d in a.disciplinas]
        ) for a in items
    ]
...
def create_student(body: AlunoIn, token: str = Depends(require_token)):
    try:
        a = db.create_aluno(body.nome, body.tipo_id, body.identificador, body.data_cadastro)
        return AlunoOut(id=a.id, nome=a.nome, tipo_id=a.tipo_id, identificador=a.identificador, data_cadastro=a.data_cadastro, disciplinas=[])
    except ValueError as e:
        raise HTTPException(400, str(e))
...
def update_student(aid: str, body: AlunoIn, token: str = Depends(require_token)):
    try:
        a = db.update_aluno(aid, nome=body.nome, tipo_id=body.tipo_id, identificador=body.identificador, data_cadastro=body.data_cadastro)
        return AlunoOut(
            id=a.id, nome=a.nome, tipo_id=a.tipo_id, identificador=a.identificador, data_cadastro=a.data_cadastro,
            disciplinas=[DisciplinaOut(id=d.id, nome=d.nome, data_cadastro=d.data_cadastro, notas=d.notas, media=d.media(), status=d.status()) for d in a.disciplinas]
        )
...
def delete_student(aid: str, token: str = Depends(require_token)):
    try:
        db.delete_aluno(aid); return {"ok": True}
    except ValueError as e:
        raise HTTPException(404, str(e))
...
def list_courses(
    aid: str,
    name: Optional[str] = None,
    stage_with_grade: Optional[str] = None,
    date_min: Optional[str] = None,
    date_max: Optional[str] = None,
    token: str = Depends(require_token)
):
    ...
    if nonempty(date_min):
        ensure_date(date_min); ds = [d for d in ds if to_date(d.data_cadastro) >= to_date(date_min)]
    if nonempty(date_max):
        ensure_date(date_max); ds = [d for d in ds if to_date(d.data_cadastro) <= to_date(date_max)]
    return [DisciplinaOut(id=d.id, nome=d.nome, data_cadastro=d.data_cadastro, notas=d.notas, media=d.media(), status=d.status()) for d in ds]
...
def delete_course(aid: str, did: str, token: str = Depends(require_token)):
    try:
        db.del_disciplina(aid, did); return {"ok": True}
    except ValueError as e:
        raise HTTPException(404, str(e))
...
def aluno_csv(aid: str, token: str = Depends(require_token)):
    ...
    sio = StringIO()
    w = csv.writer(sio)
    w.writerow(["Aluno", a.nome]); w.writerow(["Identificador", f"{a.tipo_id}: {a.identificador}"])
    w.writerow(["Cadastro", a.data_cadastro]); w.writerow([])
...
def turma_csv(token: str = Depends(require_token)):
    ...
    w.writerow(["Aluno","Tipo","Identificador","Disciplina","E1","E2","E3","Média","Status","Cadastro"])
    for a in alunos:
        if not a.disciplinas:
            w.writerow([a.nome,a.tipo_id,a.identificador,"(sem disciplinas)","","","","","EM CURSO",a.data_cadastro])
        for d in a.disciplinas:
            w.writerow([a.nome,a.tipo_id,a.identificador,d.nome,d.notas.get("E1"),d.notas.get("E2"),d.notas.get("E3"),d.media(),d.status(),d.data_cadastro])
    sio.seek(0)
    return StreamingResponse(iter([sio.getvalue()]), media_type="text/csv",
